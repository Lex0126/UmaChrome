...
try:
    ...
    TF_AVAILABLE = False
    # Si tensorflow no está instalado, aún intentaremos crear un archivo con pesos (pickle) si se desea.
    import pickle
...
STAGNATION_LIMIT = 10  # generaciones sin mejora para disparar exportación
TRAIN_SAMPLES = 10000  # muestras sintéticas para entrenar el Keras
TRAIN_EPOCHS = 10
...
# ---------------- FUNCIONES PARA EXPORTAR A KERAS ----------------
def cerebro_to_numpy(cerebro):
    # Convierte las listas a arrays numpy con shape correcto
    w_ih = np.array(cerebro.w_ih, dtype=np.float32)  # shape (2,6)
    w_ho = np.array(cerebro.w_ho, dtype=np.float32).reshape((6, 1))  # shape (6,1)
    bias_h = np.array(cerebro.bias_h, dtype=np.float32)  # shape (6,)
    bias_o = np.array([cerebro.bias_o], dtype=np.float32)  # shape (1,)
    return w_ih, bias_h, w_ho, bias_o
...
def init_model_weights_from_cerebro(model, cerebro):
    # Tratar de inicializar pesos del modelo con los del cerebro
    w_ih, bias_h, w_ho, bias_o = cerebro_to_numpy(cerebro)
    # Capa 0: Dense(6) kernel shape (2,6) bias (6,)
    # Capa 1: Dense(1) kernel shape (6,1) bias (1,)
    try:
        model.layers[0].set_weights([w_ih, bias_h])
        model.layers[1].set_weights([w_ho, bias_o])
        return True
    except Exception as e:
        print("No fue posible inicializar pesos en Keras:", e)
        return False

def generate_training_data_from_cerebro(cerebro, n_samples=TRAIN_SAMPLES):
    # Genera entradas aleatorias dentro de los rangos (distancia, velocidad)
    X = np.zeros((n_samples, 2), dtype=np.float32)
    y = np.zeros((n_samples, 1), dtype=np.float32)
    for i in range(n_samples):
        distancia = random.uniform(0, ANCHO)   # rango del input distancia
        velocidad = random.uniform(0, 25)      # rango de velocidad
        X[i, 0] = distancia / ANCHO
        X[i, 1] = velocidad / 20
        decision = cerebro.pensar(distancia, velocidad)
        y[i, 0] = 1.0 if decision else 0.0
    return X, y

def export_cerebro_to_keras(cerebro, path=MODEL_SAVE_PATH, train=True):
    """
    Crea un modelo Keras que imite al cerebro dado. Si TF está disponible, intenta entrenar
    con datos sintéticos etiquetados por el cerebro. Guarda el modelo en `path`.
    """
    print("Exportando cerebro a Keras. TensorFlow disponible:", TF_AVAILABLE)
    model = build_keras_model()

    # Inicializar pesos desde el cerebro (mejora convergencia)
    init_ok = init_model_weights_from_cerebro(model, cerebro)

    if TF_AVAILABLE and train:
        X, y = generate_training_data_from_cerebro(cerebro, n_samples=TRAIN_SAMPLES)
        print("Entrenando Keras para imitar al mejor cerebro... (muestras:", len(X), "epochs:", TRAIN_EPOCHS, ")")
        model.fit(X, y, epochs=TRAIN_EPOCHS, batch_size=32, verbose=1)
        model.save(path)
        print("Modelo Keras entrenado y guardado en:", path)
    else:
        # Si TF no está disponible, guardamos una representación simple de los pesos
        if TF_AVAILABLE:
            model.save(path)
            print("TensorFlow disponible pero train=False. Modelo guardado en:", path)
        else:
            # Guardar pesos de cerebro en pickle para poder reconstituirlos luego
            with open(path + ".pkl", "wb") as f:
                pickle.dump({
                    'w_ih': cerebro.w_ih,
                    'bias_h': cerebro.bias_h,
                    'w_ho': cerebro.w_ho,
                    'bias_o': cerebro.bias_o
                }, f)
            print("TensorFlow no está instalado. Pesos guardados en:", path + ".pkl")
    ...
    while True:
        ...
        while ejecutando:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_s:
                        # Guardar/exportar modelo cuando el usuario presiona S
                        # Tomamos el mejor cerebro de la generación actual (por seguridad tomamos el mejor de lista_umas)
                        mejores = sorted(lista_umas, key=lambda u: u.fitness, reverse=True)
                        if mejores:
                            print("Guardando modelo solicitado por usuario (tecla S)...")
                            export_cerebro_to_keras(mejores[0].cerebro)
                            print("Exportación manual completada.")
            ...
            info = [
                ...
                f"Score: {puntuacion}",
                "Presiona 'S' para exportar el mejor modelo ahora"
            ]
        ...
        # Actualización de semilla si mejora sustancial
        if puntuacion > 2000:
            semilla_actual += 1

        # Guardar historial de best scores para detectar estancamiento
        best_history.append(puntuacion)
        if puntuacion > last_best_score:
            last_best_score = puntuacion
            generations_since_improve = 0
        else:
            generations_since_improve += 1

        # Si llevamos muchas generaciones sin mejora, exportamos el mejor cerebro
        if generations_since_improve >= STAGNATION_LIMIT:
            # determinar mejor cerebro de la población anterior (lista_umas)
            mejores = sorted(lista_umas, key=lambda u: u.fitness, reverse=True)
            if mejores:
                print(f"No hubo mejora en {generations_since_improve} generaciones. Exportando mejor cerebro...")
                export_cerebro_to_keras(mejores[0].cerebro)
            generations_since_improve = 0  # resetear contador tras exportación

        # Reproducir siguiente generación
        lista_umas, mejor_uma = (lambda pop: (siguiente_generacion(pop)[0], siguiente_generacion(pop)[1]))(lista_umas)
        # Nota: la línea anterior usa la función siguiente_generacion y toma la nueva población y el mejor de la anterior.
        # Reescribimos el grupo de sprites
        grupo_umas.empty()
        for u in lista_umas: grupo_umas.add(u)
        generacion_actual += 1
